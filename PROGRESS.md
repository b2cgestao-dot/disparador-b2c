...
- Tarefa em foco: (nenhuma) - construcao CONCLUIDA e EM PRODUCAO. T0..T11 DONE. Painel: https://disparador.b2cgestao.com.br
...
- T10 | DONE | 2026-08-26 15:10 | DEPLOY REAL FEITO (Dokploy na VPS Hostinger 179.199.133.18, app a partir do GitHub com o Dockerfile da raiz, dominio disparador.b2cgestao.com.br via Cloudflare DNS-only + Let's Encrypt do Traefik). Webhook da Meta registrado via Graph API (POST /{app_id}/subscriptions com app token) e ativo. Testar/Inscrever/Sync validados contra a Meta real pelo backend em producao. Historico (25/08): Artefatos PRONTOS e validados (server/Dockerfile node:22-alpine, docker-compose.prod.yml api+caddy com healthcheck/restart, Caddyfile com no-cache na raiz / + proxy /api/* /whatsapp/* /health, DEPLOY.md runbook scp + compose up -d --build + checklist go-live). `docker compose config` e `caddy validate` passam (7 testes verdes). Deploy real BLOCKED: aguardando VPS + dominio + DNS do dono (CREDENTIALS-TODO.md).
